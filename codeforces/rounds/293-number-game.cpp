 ...
 * Verdict:  ACCEPTED        Solved: 2021-05-18
 * Language: C++17 (GCC 7-32)
 * Runtime:  31 ms     Memory: 0 KB
 * Tags:     games, math, number theory
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1370/submission/116525104
 */
...
tc{
	int n;
	cin>>n;
	if(n==1){ // first player wont be able to do anything
		cout<<"FastestFinger\n";
	} 
	else if((n&1)||n==2){ //first subtract or divide and win
		cout<<"Ashishgup\n";
	} else if((n&(n-1)) == 0){ // first will subtract, then other player will divide by itself and he will win
		cout<<"FastestFinger\n";
	} else { // n must have at least one odd divisor and is even at this point
		if(n%4==0){
			cout<<"Ashishgup\n";
		} else {
			if(isPrime(n/(1ll<<__builtin_ctz(n)))){ // basically checks if odd divisor count is 1 cuz odddivisorcount function is broken
				cout<<"FastestFinger\n";
			} else cout<<"Ashishgup\n";
		}
	}
