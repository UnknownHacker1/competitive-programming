...
tc{
	...
	ll ans=1e18,mnodd=a[1],mneven=2e9,cursum=a[1],ecnt=0,ocnt=1;
	for(int i = 2; i <= n; i++){
		if(i%2==1)mnodd=min(mnodd,a[i]),++ocnt;
		if(i%2==0)mneven=min(mneven,a[i]),++ecnt;
		cursum+=a[i];
		ans=min(ans,cursum-mneven-mnodd+mneven*(n-ecnt+1)+mnodd*(n-ocnt+1));
	}
	cout<<ans<<'\n';
}
    return 0;
