/*
 * Circle of Monsters  [1334C]
 * Problem:  https://codeforces.com/problemset/problem/1334/C
 * Verdict:  ACCEPTED        Solved: 2021-09-03
 * Language: C++17 (GCC 7-32)
 * Runtime:  342 ms     Memory: 10700 KB
 * Tags:     brute force, constructive algorithms, greedy, math
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1334/submission/127786957
 */
...
tc{
	...
	ll pref[n+1]={0};
	pref[1]=max(0ll,a[0]-b[n-1]);
	for(int i = 2; i <= n; i++){
		pref[i]=pref[i-1]+max(0ll,a[i-1]-b[i-2]);
	}
	ll ans=1e18;
	for(int i = 1; i <= n; i++){
		ans=min(ans,a[i-1]+pref[n]-pref[i]+pref[i-1]);
	}
	cout<<ans<<'\n';
}
}