/*
 * Good Numbers (hard version)  [1249C2]
 * Problem:  https://codeforces.com/problemset/problem/1249/C2
 * Verdict:  ACCEPTED        Solved: 2021-05-23
 * Language: C++17 (GCC 7-32)
 * Runtime:  15 ms     Memory: 0 KB
 * Tags:     binary search, greedy, math, meet-in-the-middle
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1249/submission/117125024
 */
...
tc{
	...
	ll n_copy=n;
	while(n){
		ternary_rep.push_back((n%3==2?0:n%3)+'0');
		if(n%3==1){
			ans+=POW(3,cur);
		}
		n/=3,++cur;
	...
	bool found=0;
	for(int i = 0; i < (int)ternary_rep.size(); i++){
		if(ans>=n_copy){
			found=1;
			break;
		}
	...
	for(int i = (int)ternary_rep.size()-1; i >= 0; i--){
		if(ans-POW(3,i)>=n_copy)
			ans-=POW(3,i);
	}
	if(!found)ans=POW(3,cur);
	cout<<ans<<'\n';
}
}