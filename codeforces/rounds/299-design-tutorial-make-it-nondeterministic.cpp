/*
 * Design Tutorial: Make It Nondeterministic  [472C]
 * Problem:  https://codeforces.com/problemset/problem/472/C
 * Verdict:  ACCEPTED        Solved: 2021-05-11
 * Language: C++17 (GCC 7-32)
 * Runtime:  218 ms     Memory: 31500 KB
 * Tags:     greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/472/submission/115942887
 */
...
int main() {
	...
	string prev=min(f[p[0]],s[p[0]]);
	bool can=1;
	vector<string>debug;debug.push_back(prev);
	for(int i = 1; i < n; i++){
		bool can1=(f[p[i]] > prev);
		bool can2=(s[p[i]] > prev);
		if(i+1<n){
			can1 &= (f[p[i]] < max(f[p[i+1]], s[p[i+1]]));
			can2 &= (s[p[i]] < max(f[p[i+1]], s[p[i+1]]));
		}
		if(can1&&can2){
