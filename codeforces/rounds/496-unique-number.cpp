 ...
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1462/submission/116271260
 */
...
void rec(int idx, int val, vector<int>vc){
	if(val==0){
		if((vc.size()==ans.size()?vc<ans:vc.size()<ans.size()))
			ans=vc;
		return;
	}
	if(idx==9){
	...
	vc.push_back(arr[idx]);
	rec(idx+1,val-arr[idx],vc);
	vc.pop_back();
	rec(idx+1,val,vc);
}
int main() {
