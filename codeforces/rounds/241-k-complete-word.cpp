 ...
 * Verdict:  ACCEPTED        Solved: 2021-05-22
 * Language: C++17 (GCC 7-32)
 * Runtime:  77 ms     Memory: 400 KB
 * Tags:     dfs and similar, dsu, greedy, implementation, strings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1332/submission/117050472
 */
...
tc{
	...
	for(int i = 1; i <= n-k; i++){
		...
		char best=max_element(freq,freq+26)-freq+'a';
		y=0;
		while(i+k*y<=n){
			if(s[i+k*y]!=best)
				s[i+k*y]=best,++ans;
			if(s[n+1-(i+k*y)]!=best)
				s[n+1-(i+k*y)]=best,++ans;
			++y;
		}
	}
	cout<<ans<<'\n';
