/*
 * Cellular Network  [702C]
 * Problem:  https://codeforces.com/problemset/problem/702/C
 * Verdict:  ACCEPTED        Solved: 2021-05-21
 * Language: C++17 (GCC 7-32)
 * Runtime:  343 ms     Memory: 1900 KB
 * Tags:     binary search, implementation, two pointers
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/702/submission/116931011
 */
...
int main() {
	...
	for(auto&i:b)cin>>i;
	sort(a,a+n);
	ll lo=0,hi=2e9; // worst case m = 1, n > 1, b[0]=-1e9, min(a)=-1e9,max(a)=1e9;
	ll ans=hi;
	while(lo<=hi){
		ll mid=(lo+hi)/2;
		int cvarr[n+5]={0};//cv = cover
		for(int i = 0; i < m; i++){
			if(b[i]-mid>a[n-1])continue; // guarantee cvbg will never = n
			if(b[i]+mid<a[0])continue; // i think this is useless but im adding it for safety
			int 
			cvbg=lower_bound(a,a+n,b[i]-mid)-a,
			cvend=upper_bound(a,a+n,b[i]+mid)-a-1;
		...
		if(ok)hi=mid-1,ans=min(ans,mid);
		else lo=mid+1;
	}
	cout<<ans;
}
