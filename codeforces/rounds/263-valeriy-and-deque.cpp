/*
 * Valeriy and Deque  [1179A]
 * Problem:  https://codeforces.com/problemset/problem/1179/A
 * Verdict:  ACCEPTED        Solved: 2021-05-04
 * Language: C++17 (GCC 7-32)
 * Runtime:  374 ms     Memory: 6400 KB
 * Tags:     data structures, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1179/submission/115076149
 */
...
int main() {
    ...
    ll mx=0;
    for(int i = 0; i < n; i++) {
        cin>>x;lst.push_back(x);
        mx=max(mx,x);
    }
    ll a=*lst.begin(),b=*(next(lst.begin()));
    ll qrycnt=1;
    pair<ll,ll>qryans[2*n+3];
    qryans[qrycnt]=make_pair(a,b);
    while(a!=mx) {
        if(a>b){
        ...
        a=*lst.begin(),b=*(next(lst.begin()));
        qryans[++qrycnt]=make_pair(a,b);
    }
    ll arr[n+3];
    int idx=1;auto it=lst.begin();
    for(;it!=lst.end();++idx,++it){
        arr[idx]=*it;
    }
    while(q--) {
        ll query;
        cin>>query;
        if(query<qrycnt){
            cout<<qryans[query].first<<' '<<qryans[query].second;
        }
        else{
            cout<<mx<<' '<<arr[2+((query-qrycnt)%(n-1))];
        }
        cout<<'\n';
    }
}
