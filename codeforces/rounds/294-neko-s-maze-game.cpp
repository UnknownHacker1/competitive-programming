/*
 * NEKO's Maze Game  [1292A]
 * Problem:  https://codeforces.com/problemset/problem/1292/A
 * Verdict:  ACCEPTED        Solved: 2021-05-15
 * Language: C++17 (GCC 7-32)
 * Runtime:  77 ms     Memory: 500 KB
 * Tags:     data structures, dsu, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1292/submission/116299518
 */
...
return
!(is[x][y]&&(is[x-1][y+1]||is[x+1][y-1]||is[x+1][y+1]||is[x-1][y-1]||is[x-1][y]||is[x+1][y]));
}
...
int main() {
	...
	while(q--){
		...
		cin>>x>>y;
		if(is[x][y]){
			badcnt+= // the ones that were already good
			(good(x,y)+good(x-1,y+1)+good(x+1,y-1)+good(x+1,y+1)+good(x-1,y-1)+good(x-1,y)+good(x+1,y));
			is[x][y]=0;
			badcnt-= // the ones that were bad before cuz of what we did above
			(good(x,y)+good(x-1,y+1)+good(x+1,y-1)+good(x+1,y+1)+good(x-1,y-1)+good(x-1,y)+good(x+1,y));
			if(badcnt>0){
				cout<<"No\n";
			} else cout<<"Yes\n";
			...
			badcnt-= // the ones that were already bad
			(bad(x,y)+bad(x-1,y+1)+bad(x+1,y-1)+bad(x+1,y+1)+bad(x-1,y-1)+bad(x-1,y)+bad(x+1,y));
			is[x][y]=1;
			badcnt+= // the ones that were good before cuz of what we did above
			(bad(x,y)+bad(x-1,y+1)+bad(x+1,y-1)+bad(x+1,y+1)+bad(x-1,y-1)+bad(x-1,y)+bad(x+1,y));
			if(badcnt>0){
				cout<<"No\n";
			} else cout<<"Yes\n";
