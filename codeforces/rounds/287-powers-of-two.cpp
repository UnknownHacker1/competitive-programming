/*
 * Powers Of Two  [1095C]
 * Problem:  https://codeforces.com/problemset/problem/1095/C
 * Verdict:  ACCEPTED        Solved: 2021-08-12
 * Language: C++17 (GCC 7-32)
 * Runtime:  46 ms     Memory: 5200 KB
 * Tags:     bitmasks, greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1095/submission/125700513
 */
...
int main()
{
	...
	if(k<lol.size()){
		...
		bool ans=1;
		while(lol.size()<k&&ans){
			bool ok=0;
			for(int i = 0; i < (int)lol.size() && lol.size()<k; i++){
				if(lol[i]>1){
					ok=1;
					lol.push_back(lol[i]/2);
					lol[i]/=2;
				}
			}
			ans&=ok;
