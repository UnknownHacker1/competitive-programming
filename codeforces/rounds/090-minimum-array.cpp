 ...
 * Verdict:  ACCEPTED        Solved: 2021-09-02
 * Language: C++17 (GCC 7-32)
 * Runtime:  295 ms     Memory: 18700 KB
 * Tags:     binary search, data structures, greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1157/submission/127713642
 */
...
ll st[(int)8e5],a[(int)2e5];
ll build(int l, int r, int ni){
    if(l==r){
        return st[ni]=a[l];
    }
    int mid=(l+r)/2;
    return 
    st[ni]=gcd(build(l,mid,2*ni+1),
        build(mid+1,r,2*ni+2));
}
ll query(int l, int r, int ni, int ql, int qr){
    if(l==ql&&r==qr)
        return st[ni];
    if(r<ql||l>qr)
        return 0ll;
    int mid=(l+r)/2;
    return 
    gcd(query(l,mid,2*ni+1,ql,min(qr,mid)),
        query(mid+1,r,2*ni+2,max(ql,mid+1),qr));
}
int main()
{
	ACPLS();
	...
	for(int i = 0; i < n; i++){
		...
		if(((a[i]+*st.lower_bound(n-a[i]))%n)<((a[i]+*st.begin())%n)){
			cout<<(a[i]+*st.lower_bound(n-a[i]))%n<<' ';
			st.erase(st.lower_bound(n-a[i]));
		}
		else{
			cout<<(a[i]+*st.begin())%n<<' ';
			st.erase(st.begin());
