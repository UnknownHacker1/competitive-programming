/*
 * Curiosity Has No Limits  [1031B]
 * Problem:  https://codeforces.com/problemset/problem/1031/B
 * Verdict:  ACCEPTED        Solved: 2021-08-23
 * Language: C++17 (GCC 7-32)
 * Runtime:  93 ms     Memory: 14600 KB
 * Tags:     n/a
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1031/submission/126761071
 */
...
short dp[(int)1e5+3][4];
bool rec(int idx, int t){
	if(idx+1==n)
		return 1;
	if(dp[idx][t]!=-1)
		return dp[idx][t];
	bool ret=0;
	for(int i = 0; i <= 3; i++){
		if(((t&i)==b[idx])&&((t|i)==a[idx])){
			ret|=rec(idx+1,i);
		}
	}
	return dp[idx][t]=ret;
}
void build(int idx, int t){
	if(idx+1==n)
		return;
	for(int i = 0; i <= 3; i++){
		if(((t&i)==b[idx])&&((t|i)==a[idx])&&(rec(idx+1,i))){
			cout<<i<<' ';
			build(idx+1,i);
			break;
		}
