/*
 * Permutation by Sum  [1512E]
 * Problem:  https://codeforces.com/problemset/problem/1512/E
 * Verdict:  ACCEPTED        Solved: 2021-06-16
 * Language: C++17 (GCC 7-32)
 * Runtime:  31 ms     Memory: 200 KB
 * Tags:     brute force, greedy, math
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1512/submission/119657102
 */
...
tc{
    ...
    while(v.size()<req){
    	...
    	for(int j = 0; j < (int)v.size(); j++){
    		...
    		auto&x=v[j];
    		for(int i = 1; i <= n; i++){
    			if(x>i&&is[x-i]+is[i]==0&&x!=2*i){
    				int xi=x-i;
    				is[x]=0,is[xi]=1,is[i]=1;
    				x=xi,v.push_back(i);
    				curok=1;
    				break;
    			}
