 ...
 * Problem:  https://codeforces.com/problemset/problem/574/B
 * Verdict:  ACCEPTED        Solved: 2021-06-28
 * Language: C++17 (GCC 7-32)
 * Runtime:  31 ms     Memory: 16000 KB
 * Tags:     brute force, dfs and similar, graphs, hashing
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/574/submission/120766533
 */
...
int main()
{
    ...
    int ans=1e9;
    for(int i = 0; i < m; i++){
    	for(int j = 0; j < (int)adj[lol[i].first].size(); j++){
    		if(edge[lol[i].second][adj[lol[i].first][j]]){
    			if(adj[lol[i].first].size()<2)continue;
    			if(adj[lol[i].second].size()<2)continue;
    			if(adj[adj[lol[i].first][j]].size()<2)continue;
    			ans=min(ans,
    			int(adj[lol[i].first].size()
    			+adj[lol[i].second].size()
    			+adj[adj[lol[i].first][j]].size()-6));
    		}
    	}
    }
    if(ans==1e9)ans=-1;
