 ...
 * Verdict:  ACCEPTED        Solved: 2021-06-10
 * Language: C++17 (GCC 7-32)
 * Runtime:  30 ms     Memory: 200 KB
 * Tags:     brute force, dp, greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1538/submission/118983849
 */
...
vector<int>a(105);
int dp[105][105][2][2];
int rec(int l, int r, bool mxdest, bool mndest){
	if(mxdest&&mndest){
		return (l-1)+(n-r);
	}
	if(l>r){
	...
	if(dp[l][r][mxdest][mndest]!=-1)
		return dp[l][r][mxdest][mndest];
	return dp[l][r][mxdest][mndest]=min(rec(l+1,r,mxdest||a[l]==mx,mndest||a[l]==mn),
			   rec(l,r-1,mxdest||a[r]==mx,mndest||a[r]==mn));
}
int main()
{
    ACPLS();
tc{
	...
	for(int i = 1; i <= n; i++)
		cin>>a[i];
	mx=*max_element(a.begin()+1,a.begin()+n+1);
