/*
 * Cow and Message  [1307C]
 * Problem:  https://codeforces.com/problemset/problem/1307/C
 * Verdict:  ACCEPTED        Solved: 2021-07-05
 * Language: GNU C++11
 * Runtime:  124 ms     Memory: 1300 KB
 * Tags:     brute force, dp, math, strings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1307/submission/121432082
 */
...
int main()
{
	...
	for(int a = 0; a < 26; a++){
		for(int b = 0; b < 26; b++){
			if(occ[b].empty()||occ[a].empty())
				continue;
			ll res=0;
			for(int i = 0; i < (int)occ[a].size(); i++){
				res+=int(occ[b].size())-int(upper_bound(all(occ[b]),occ[a][i])-occ[b].begin());
				ans=max(ans,res);
			}
		}
	}
