/*
 * Add to Neighbour and Remove  [1462D]
 ...
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1462/submission/116275959
 */
...
tc{
	...
	for(int i = 1; i <= n; i++){
		ll sum=0;
		ll wanted=prefixsum[i];
		bool can=1;
		int operations=0;
		int curoperations=0;
		for(int j = 1; j <= n; j++){
			sum+=a[j], ++curoperations;
			if(sum>wanted){
				can=0;
				break;
			}
			else if(sum==wanted){
				operations+=curoperations-1;
				sum=0,curoperations=0;
			}
		}
		can=(sum==0);
		if(can)ans=min(ans,operations);
	}
	cout<<ans<<'\n';
}
}