...
vector<ll> a(300005);
ll dp[300005][2][2];
ll recurse(int idx, bool take, bool negative){
    if(idx>=n)return 0;
    if(dp[idx][take][negative]!=-1)return dp[idx][take][negative];
    ll ret=0;
    if(take){
        ret+=(negative?-a[idx]:a[idx]);
    }
    bool nextTake=(take?(!negative):negative);
    return dp[idx][take][negative]=
    max(recurse(idx+1,0,nextTake),recurse(idx+1,1,nextTake))+ret;
}
int main() {
    fast
tc{
    ...
    cout<<max(recurse(0,0,0),recurse(0,1,0))<<'\n';
}
}
