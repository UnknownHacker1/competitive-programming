 ...
 * Problem:  https://codeforces.com/problemset/problem/868/C
 * Verdict:  ACCEPTED        Solved: 2021-06-03
 * Language: C++17 (GCC 7-32)
 * Runtime:  109 ms     Memory: 400 KB
 * Tags:     bitmasks, brute force, constructive algorithms, dp
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/868/submission/118289641
 */
...
bool is[1<<4]={0};
int k;
bool rec(int idx, int val){
	if(idx==k)
		return is[val];
	int op1=val,op2=val-(( !!(val&(1<<idx)) )*1<<idx);
	return rec(idx+1,op1)||rec(idx+1,op2);
}
int main(){
	ACPLS(); 
	int n;
