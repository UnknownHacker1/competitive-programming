/*
 * Magic Powder - 2  [670D2]
 * Problem:  https://codeforces.com/problemset/problem/670/D2
 * Verdict:  ACCEPTED        Solved: 2021-07-13
 * Language: GNU C++11
 * Runtime:  343 ms     Memory: 5400 KB
 * Tags:     binary search, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/670/submission/122381509
 */
...
int main()
{
	...
	for(auto&i:a)cin>>i;
	for(auto&i:b)cin>>i;
	uint64_t lo=0,hi=2000000000,ans=lo;
	while(lo<=hi){
		uint64_t mid=(lo+hi)/2,need=0;
		bool bruh=0;
		uint64_t mxneed=0;
		for(int i = 0; i < n; i++){
			if(mul_overflow(a[i],mid)){
				bruh=1;
				break;
			}
			if(a[i]*mid>b[i]){
