 ...
 * Verdict:  ACCEPTED        Solved: 2021-08-06
 * Language: C++17 (GCC 7-32)
 * Runtime:  124 ms     Memory: 6300 KB
 * Tags:     greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1046/submission/125004316
 */
...
int main()
{
	...
	reverse(all(p));
	sum+=*max_element(all(p));
	p.erase(max_element(all(p)));
	int lo=1,hi=d,ans=d;
	while(lo<=hi){
		int mid=(lo+hi)/2;
		bool ok=1;
		for(int i = mid-1,j=0; i < n-1; i++,j++){
			ok&=s[i]+p[j]<=sum;
		}
		if(ok){
			ans=mid;
