...
tc{
	...
	while(ms.size()>1){
		...
		ms.erase(ms.begin());
		mx--,mn--;
		if(mx>0)ms.insert(mx);
		if(mn>0)ms.insert(mn);
	}
	int ans=0;
	for(auto&i:ms)ans+=i;
	cout<<ans<<'\n';
