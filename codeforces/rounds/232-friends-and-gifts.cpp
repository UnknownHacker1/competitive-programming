/*
 * Friends and Gifts  [1283C]
 * Problem:  https://codeforces.com/problemset/problem/1283/C
 * Verdict:  ACCEPTED        Solved: 2021-06-04
 * Language: C++17 (GCC 7-32)
 * Runtime:  202 ms     Memory: 9500 KB
 * Tags:     constructive algorithms, data structures, math
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1283/submission/118318808
 */
...
int main(){
	...
	for(int i = 0; i < n; i++){
		if(a[i]==0&&st.find(i+1)!=st.end()){
			vc.push_back(i+1);
		}
	}
	int vcidx=(int)vc.size()-1;
	for(int i = 0; i < n; i++){
		if(a[i]==0){
			...
			else a[i]=*st.begin(),st.erase(st.begin());
		}
	}
	int idx=-1;
	for(int i = 0; i < n; i++){
	...
	if(idx!=-1){
		for(int i = 0; i < n; i++){
			if(i==idx)continue;
			if(!iszeroidx[i])continue;
			if(a[i]!=idx+1&&a[idx]!=i+1){
				swap(a[i],a[idx]);
				break;
			}
		}
