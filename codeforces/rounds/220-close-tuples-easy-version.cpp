/*
 * Close Tuples (easy version)  [1462E1]
 * Problem:  https://codeforces.com/problemset/problem/1462/E1
 * Verdict:  ACCEPTED        Solved: 2021-06-24
 * Language: C++17 (GCC 7-32)
 * Runtime:  109 ms     Memory: 12700 KB
 * Tags:     binary search, combinatorics, math, sortings, two pointers
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1462/submission/120405068
 */
...
ret dist(ret X1, ret Y1, ret X2, ret Y2)
{
...
vector<vector<ll>> C(2e5+3,vector<ll>(5,0));
void bin_coeff(int n, int k)
{
    for(int i = 0; i <= k; i++) C[i][i]=1, C[i][0]=1;
...
tc{
	...
	for(int i = 1; i <= n; i++){
		for(int j = 0; j < 9; j++){
			...
			int x=i+(j%3),y=i+((j/3)%3),z=i+((j/9)%3);
			if(x==y&&y==z){
				ans+=C[freq[x]][3];
			}
			else if(x==y){
				ans+=C[freq[x]][2]*freq[z];
			}
			else if(y==z){
				ans+=C[freq[y]][2]*freq[x];
			}
			else if(x==z){
				ans+=C[freq[x]][2]*freq[y];
			}
			else{
