/*
 * Minimum Value Rectangle  [1027C]
 * Problem:  https://codeforces.com/problemset/problem/1027/C
 * Verdict:  ACCEPTED        Solved: 2021-08-28
 * Language: C++17 (GCC 7-32)
 * Runtime:  732 ms     Memory: 7600 KB
 * Tags:     greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1027/submission/127228549
 */
...
tc{
	...
	vector<int>a;
	bool vis[(int)1e4+3]={0};
	for(int i = 0; i < n; i++){
		if(freq[b[i]]>=2&&vis[b[i]]==0)
			a.push_back(b[i]),vis[b[i]]=1;
	}
	sort(all(a));
	int x=-1,y=-1;
	long double bst=-1;
	for(int i = 1; i < (int)a.size(); i++){
		long double res=1.0*((2.0*(1.0*a[i]+a[i-1]))*(2.0*(1.0*a[i]+a[i-1])))/(1.0*a[i]*a[i-1]);
		if(x==-1||res<bst){
			x=a[i-1],y=a[i];
			bst=res;
		}
