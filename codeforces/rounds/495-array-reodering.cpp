...
tc{
	...
	vector<int>vc;
	for(auto&i:a){
		if(i%2==0)vc.push_back(i);
	}
	for(auto&i:a){
		if(i&1)vc.push_back(i);
	}
	ll ans=0;
	for(int i = 0; i < n; i++){
		for(int j = 0; j < n; j++){
			if(i<j)ans+=gcd(vc[i],2*vc[j])>1;
		}
	}
	cout<<ans<<'\n';
}
