/*
 * Pursuit  [1530C]
 * Problem:  https://codeforces.com/problemset/problem/1530/C
 * Verdict:  ACCEPTED        Solved: 2021-07-17
 * Language: C++17 (GCC 9-64)
 * Runtime:  124 ms     Memory: 11400 KB
 * Tags:     binary search, brute force, greedy, sortings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1530/submission/122838788
 */
...
tc{
	...
	while(lo<=hi){
		...
		for(auto&i:a)
			ac.push_back(i);
		for(int i = 0; i < mid; i++)
			bc.push_back(0),ac.push_back(100);
		for(int i = 0; i < n; i++){
			bc.push_back(b[i]);
		}
		for(int i = (int)ac.size()/4; i < (int)ac.size(); i++){
			sma+=ac[i];
			smb+=bc[i];
		}
		if(sma>=smb){
