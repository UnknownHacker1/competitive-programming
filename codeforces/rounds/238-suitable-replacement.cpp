/*
 * Suitable Replacement  [825D]
 * Problem:  https://codeforces.com/problemset/problem/825/D
 * Verdict:  ACCEPTED        Solved: 2021-06-07
 * Language: C++17 (GCC 7-32)
 * Runtime:  62 ms     Memory: 5000 KB
 * Tags:     binary search, greedy, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/825/submission/118743240
 */
...
int main(){
	...
	string s,t;
	cin>>s>>t;
	int freqs[26]={0},freqt[26]={0},qcnt=0;
	...
	ll lo = 0, hi = (int)s.size()+2;
	ll ans=0;
	while(lo<=hi){
		ll mid=(lo+hi)/2;
		ll totreq=0;
		for(int i = 0; i < 26; i++){
			totreq+=max(0ll,freqt[i]*mid-freqs[i]);
		}
		if(totreq<=qcnt){
			ans=max(ans,mid);
			lo=mid+1;
	...
	string best;
	for(int i = 0; i < 26; i++){
		int req=max(0ll,freqt[i]*ans-freqs[i]);
		while(req-->0){
			best.push_back(i+'a');
		}
	}
