/*
 * Did you mean...  [858C]
 * Problem:  https://codeforces.com/problemset/problem/858/C
 * Verdict:  ACCEPTED        Solved: 2021-05-19
 * Language: C++17 (GCC 7-32)
 * Runtime:  46 ms     Memory: 0 KB
 * Tags:     dp, greedy, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/858/submission/116637268
 */
...
int main() {
	...
	cin>>s;
	int n = (int)s.size();
	bool space_before[n]={0};
	int cnt=0;
	for(int i = 0; i < n; i++){
		++cnt;
		if(cnt<3)continue;
		if((!(s[i]==s[i-1]&&s[i-1]==s[i-2]))&&
			isconsonant(s[i])&&isconsonant(s[i-1])&&isconsonant(s[i-2])){
			space_before[i]=1;
			cnt=1;
		}
	}
