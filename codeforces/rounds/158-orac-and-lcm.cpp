/*
 * Orac and LCM  [1349A]
 * Problem:  https://codeforces.com/problemset/problem/1349/A
 * Verdict:  ACCEPTED        Solved: 2021-06-13
 * Language: C++17 (GCC 7-32)
 * Runtime:  234 ms     Memory: 19800 KB
 * Tags:     data structures, math, number theory
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1349/submission/119404561
 */
...
int main()
{
    ...
    for(auto x:a){
    	...
    	if(cnt2)mp[2]++,mp1[2].insert(cnt2),++freq[{2,cnt2}];
    	for(int i = 3; i * i <= x; i+=2){
    		...
    		if(f)mp[i]++,mp1[i].insert(cnt),++freq[{i,cnt}];
    	}
    	if(x>2)mp[x]++,mp1[x].insert(1),++freq[{x,1}];
    }
    for(auto&i:mp){
    	if(i.second==n){
    		if(freq[{i.first,*mp1[i.first].begin()}]>1)
    			ans*=POWNOM(i.first,*mp1[i.first].begin());
    		else ans*=POWNOM(i.first,*next(mp1[i.first].begin()));
    	}
    	else if(i.second==n-1){
    		ans*=POWNOM(i.first,*mp1[i.first].begin());
    	}
    }
    cout<<ans;
