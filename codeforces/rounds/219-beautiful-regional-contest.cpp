/*
 * Beautiful Regional Contest  [1264A]
 * Problem:  https://codeforces.com/problemset/problem/1264/A
 * Verdict:  ACCEPTED        Solved: 2021-07-01
 * Language: GNU C++11
 * Runtime:  421 ms     Memory: 28800 KB
 * Tags:     greedy, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1264/submission/121042336
 */
...
#define sum(l,r,pref) (l==0?pref[r]:pref[r]-pref[l-1])
int main()
{
    ACPLS();
tc{
	...
	for(int i = 0; i < n; i++){
		if(vis[a[i]])continue;
		vis[a[i]]=1;
		vc.push_back(freq[a[i]]);
	}
	int sumvc=accumulate(all(vc),0);
	while(sumvc>n/2){
		sumvc-=vc.back();
		vc.pop_back();
	}
	...
	int g=0,s=0,b=0;
	for(int i = 1; i+1 < (int)vc.size(); i++){
		if(sum(1,i,vc)>vc[0]&&sum(i+1,(int)vc.size()-1,vc)>vc[0]){
			g=vc[0];
			s=sum(1,i,vc);
			b=sum(i+1,(int)vc.size()-1,vc);
			break;
		}
	}
	cout<<g<<' '<<s<<' '<<b<<'\n';
