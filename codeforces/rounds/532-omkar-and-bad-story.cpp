/*
 * Omkar and Bad Story  [1536A]
 * Problem:  https://codeforces.com/problemset/problem/1536/A
 * Verdict:  ACCEPTED        Solved: 2021-06-06
 * Language: C++17 (GCC 7-32)
 * Runtime:  46 ms     Memory: 200 KB
 * Tags:     brute force, constructive algorithms
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1536/submission/118613308
 */
...
tc{
	...
	while(a.size()<300){
		bool ok = 1;
		for(int i = 0; i < (int)a.size() && a.size()<300; i++){
			for(int j = 0; j < (int)a.size() && a.size()<300; j++){
				if(i==j)continue;
				auto&ref=freq[abs(a[i]-a[j])];
				ok&=ref;
				if(!ref){
					ref=1;
