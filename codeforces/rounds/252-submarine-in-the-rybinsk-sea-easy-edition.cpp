/*
 * Submarine in the Rybinsk Sea (easy edition)  [1195D1]
 * Problem:  https://codeforces.com/problemset/problem/1195/D1
 * Verdict:  ACCEPTED        Solved: 2021-05-21
 * Language: C++17 (GCC 7-32)
 * Runtime:  342 ms     Memory: 800 KB
 * Tags:     combinatorics, math, number theory
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1195/submission/116963438
 */
...
int main() {
	...
	for(int i = 0; i < n; i++){
		int lg = 0;
		int lg10=int(log10(a[i]))+1;
		ll cntodd=0,cnteven=0;
		string str=to_string(a[i]);
		while(a[i]){
			int dg=a[i]%10;
			cnteven%=mod;
			cnteven+=((n)%mod*dg%mod*POW(10,2*lg+1)%mod)%mod;//if it is b
			cnteven%=mod;
			cntodd%=mod;
			cntodd+=((n)%mod*dg%mod*POW(10,2*lg)%mod)%mod;//if it is a
			cntodd%=mod;
			++lg,a[i]/=10;
		}
		ans+=(cnteven%mod)+(cntodd%mod);
		ans%=mod;
	}
	cout<<ans;
}