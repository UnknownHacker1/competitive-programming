/*
 * Divide The Students  [1250L]
 * Problem:  https://codeforces.com/problemset/problem/1250/L
 * Verdict:  ACCEPTED        Solved: 2021-07-12
 * Language: GNU C++11
 * Runtime:  30 ms     Memory: 3800 KB
 * Tags:     binary search, greedy, math
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1250/submission/122224798
 */
...
tc{
	...
	group1=max(a,c)/2,group2=max(a,c)-group1;
	group3=min(a,c);
	if(group3<group2){
		int x = min(group2-group3,b);
		b-=x;
		group3+=x;
	}else{
		int x = min(group3-group2,b);
		b-=x;
		group2+=x;
	}
	int x = min(group2-group1,b);
	group1+=x;
	b-=x;
	cout<<max({group1,group2,group3})+ceili(b,3)<<'\n';
}
	return 0;
}