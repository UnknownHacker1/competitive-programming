/*
 * Sequence and Swaps  [1455D]
 * Problem:  https://codeforces.com/problemset/problem/1455/D
 * Verdict:  ACCEPTED        Solved: 2021-08-05
 * Language: C++17 (GCC 7-32)
 * Runtime:  264 ms     Memory: 507700 KB
 * Tags:     dp, greedy, sortings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1455/submission/124970448
 */
...
int rec(int idx, int x, int prv){
	...
	int ret=1e9;
	if(x>=prv&&a[idx]>x){
		ret=min(ret,rec(idx+1,a[idx],x)+1);
	}
	if(a[idx]>=prv){
		ret=min(ret,rec(idx+1,x,a[idx]));
	}
	return dp[idx][x][prv]=ret;
} 
int main()
{
	ACPLS();
