...
tc{
    int a,b,k;
    cin>>a>>b>>k;
    int freqb[a+3]={0},freqg[b+3]={0};
    pair<int,int> arr[k];
    for(int i = 0; i < k; i++){
        int x;cin>>x;freqb[x]++,arr[i].first=x;
    }for(int i = 0; i < k; i++) {
        int x;cin>>x;freqg[x]++,arr[i].second=x;
    }
    ll ans = 0;
    for(int i = 0; i < k; i++) {
        ans+=k-freqb[arr[i].first]-freqg[arr[i].second];
    }
    cout<<(ans+k)/2<<'\n';
}
}
