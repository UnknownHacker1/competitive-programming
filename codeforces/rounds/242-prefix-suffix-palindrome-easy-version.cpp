/*
 * Prefix-Suffix Palindrome (Easy version)  [1326D1]
 * Problem:  https://codeforces.com/problemset/problem/1326/D1
 * Verdict:  ACCEPTED        Solved: 2021-05-20
 * Language: C++17 (GCC 7-32)
 * Runtime:  46 ms     Memory: 300 KB
 * Tags:     hashing, string suffix structures, strings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1326/submission/116740913
 */
...
tc{
	...
	int pref=0,suff=n-1;
	while(s[pref]==s[suff]&&pref<suff)
		++pref,--suff;
	if(pref>=suff){
		cout<<s<<'\n';
		continue;
	}
	// look for a palindrome in pref and suff
	int prefBiggest=0,suffBiggest=0;
	for(int i = pref; i <= suff; i++){
		string tmp(s.begin()+pref,s.begin()+i+1);
		if(isPalindrome(tmp)){
			prefBiggest=i-pref;
		}
	}
	reverse(s.begin()+pref,s.begin()+suff+1);
	for(int i = pref; i <= suff; i++){
		string tmp(s.begin()+pref,s.begin()+i+1);
		if(isPalindrome(tmp)){
			suffBiggest=i-pref;
