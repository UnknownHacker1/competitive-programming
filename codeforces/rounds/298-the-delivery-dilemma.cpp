/*
 * The Delivery Dilemma  [1443C]
 * Problem:  https://codeforces.com/problemset/problem/1443/C
 * Verdict:  ACCEPTED        Solved: 2021-05-13
 * Language: C++17 (GCC 7-32)
 * Runtime:  389 ms     Memory: 24300 KB
 * Tags:     binary search, greedy, sortings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1443/submission/116069822
 */
...
tc{
	...
	cin>>n;
	ll a[n],b[n];
	map<pair<ll,int>,int>index; // index of y-th occurence of x where
	                                    // key of the map is a pair (x,y)
	map<ll,int> freq{}; // helper for the above
	for(int i = 0; i < n; i++){
		cin>>a[i];
		int&ref=freq[a[i]];
		++ref;
	...
	sort(a,a+n);
	for(int i = 0; i < n; i++){
		if(i==0||a[i]!=a[i-1])mnacnt=1;
		else ++mnacnt;
		sumb-=b[index[{a[i],mnacnt}]];
		ans=min(ans,max(sumb,a[i]));
	}
	cout<<ans<<'\n';
}
}