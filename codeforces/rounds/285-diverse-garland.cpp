/*
 * Diverse Garland  [1108D]
 * Problem:  https://codeforces.com/problemset/problem/1108/D
 * Verdict:  ACCEPTED        Solved: 2021-08-19
 * Language: C++17 (GCC 7-32)
 * Runtime:  62 ms     Memory: 35700 KB
 * Tags:     constructive algorithms, dp, greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1108/submission/126463474
 */
...
int rec(int idx, int prv){
	...
	for(int i = 0; i < 3; i++){
		if(prv!=i){
			ret=min(ret,rec(idx+1,i)+(i!=a[idx]));
		}
	}
	return dp[idx][prv+1]=ret;
}
char RGB[3]={'R','G','B'};
void build(int idx, int prv){
	if(idx==n)
		return;
	int c[3]={(int)1e9,(int)1e9,(int)1e9};
	for(int i = 0; i < 3; i++){
		if(prv!=i){
			c[i]=rec(idx+1,i)+(i!=a[idx]);
		}
	}
	int best=min_element(c,c+3)-c;
	cout<<RGB[best];
	build(idx+1,best);
}
int main()
