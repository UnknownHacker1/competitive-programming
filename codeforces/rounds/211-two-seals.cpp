/*
 * Two Seals  [837C]
 * Problem:  https://codeforces.com/problemset/problem/837/C
 * Verdict:  ACCEPTED        Solved: 2021-07-13
 * Language: GNU C++11
 * Runtime:  31 ms     Memory: 3800 KB
 * Tags:     brute force, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/837/submission/122370801
 */
...
int main()
{
	...
	for(int i = 0; i < n; i++){
		for(int j = i+1; j < n; j++){
			ans=max(ans,f(a,b,x[i],y[i],x[j],y[j]));
			ans=max(ans,f(a,b,y[i],x[i],x[j],y[j]));
			ans=max(ans,f(a,b,x[i],y[i],y[j],x[j]));
			ans=max(ans,f(a,b,y[i],x[i],y[j],x[j]));
		}
	}
	cout<<ans;
	return 0;
