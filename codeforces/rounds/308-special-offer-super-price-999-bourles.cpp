/*
 * Special Offer! Super Price 999 Bourles!  [219B]
 * Problem:  https://codeforces.com/problemset/problem/219/B
 * Verdict:  ACCEPTED        Solved: 2021-04-27
 * Language: C++17 (GCC 7-32)
 * Runtime:  62 ms     Memory: 0 KB
 * Tags:     implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/219/submission/114356249
 */
...
int main() {
    ...
    for(ll k = lg; k >= 1; k--) {
        ...
        if(diff<0)continue;
        ll res=diff/pw;
        for(int i = 0; i < k; i++)res*=10,res+=9;
        if(p-res<=d){
            int cnt=0;
            ll pcpy=p;
            while(pcpy%10==9) {
