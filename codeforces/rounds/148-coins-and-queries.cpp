/*
 * Coins and Queries  [1003D]
 * Problem:  https://codeforces.com/problemset/problem/1003/D
 * Verdict:  ACCEPTED        Solved: 2021-08-27
 * Language: C++17 (GCC 7-32)
 * Runtime:  1262 ms     Memory: 3700 KB
 * Tags:     greedy
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1003/submission/127224240
 */
...
int main()
{
	...
	while(q--){
		...
		cin>>b;
		vector<int>g=f;
		for(int i = 29,x=536870912; i >= 0; i--,x/=2){
			if(!g[i])continue;
			int lo=0,hi=1e9,res=lo;
			while(lo<=hi){
				int mid=(lo+hi)/2;
				if(b<x*mid||g[i]<mid){
					hi=mid-1;
