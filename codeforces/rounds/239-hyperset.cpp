 ...
 * Verdict:  ACCEPTED        Solved: 2021-05-31
 * Language: C++17 (GCC 7-32)
 * Runtime:  2589 ms     Memory: 106200 KB
 * Tags:     brute force, data structures, implementation
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1287/submission/118020385
 */
...
int main() {
    ...
    cin>>n>>k;
    string s[n];
    map<string,short>freq{};
    map<string,short>freq2{};
    for(int i = 0; i < n; i++){
    	cin>>s[i];
    	freq[s[i]]++;
    }
    ...
    for(short i = 0; i < n; i++){
    	for(short j = i+1; j < n; j++){
    		...
    		short&ref2=freq[res];
    		if(ref2==0)continue;
    		string lol[3]={res,s[i],s[j]};
    		sort(lol,lol+3);
    		short&ref=freq2[lol[0]+lol[1]+lol[2]];
    		++ref;
    		if(ref<=min({ref2,freq[s[i]],freq[s[j]]})){
    			ans++;
    		}
    	}
    }
