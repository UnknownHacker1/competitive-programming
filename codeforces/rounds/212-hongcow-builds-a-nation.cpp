/*
 * Hongcow Builds A Nation  [744A]
 * Problem:  https://codeforces.com/problemset/problem/744/A
 * Verdict:  ACCEPTED        Solved: 2021-06-09
 * Language: C++17 (GCC 7-32)
 * Runtime:  62 ms     Memory: 2500 KB
 * Tags:     dfs and similar, graphs
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/744/submission/118916770
 */
...
vector<int>adj[1003];
bool special[1003],vis[1003];
bool edge[1003][1003];
vector<vector<int>>sc; // special components i guess
vector<int>vc,component;
void dfs(int node){
	if(vis[node])return;
	vis[node]=1;
...
int main()
{
    ACPLS();
   	...
   	int mxsz=0;
   	for(auto&v:sc)mxsz=max(mxsz,(int)v.size());
   	for(auto&v:sc){
   		if((int)v.size()==mxsz){
   			append(v,vc);
   			break;
