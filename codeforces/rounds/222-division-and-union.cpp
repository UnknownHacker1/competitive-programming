/*
 * Division and Union  [1101C]
 * Problem:  https://codeforces.com/problemset/problem/1101/C
 * Verdict:  ACCEPTED        Solved: 2021-06-27
 * Language: C++17 (GCC 7-32)
 * Runtime:  109 ms     Memory: 3100 KB
 * Tags:     sortings
 * Author:   BidoTeima
 * Source:   https://codeforces.com/contest/1101/submission/120765775
 */
...
tc{
	...
	vector<pair<ll,ll>>copy(a,a+n);
	sort(a,a+n);
	vector<pair<ll,ll>>un;
	pair<ll,ll>p={a[0].first,a[0].second};
	for(int i = 1; i < n; i++){
		if(intersect(p,a[i])){
			p={min(p.first,a[i].first),max(p.second,a[i].second)};
		}
		else{
			un.push_back(p);
